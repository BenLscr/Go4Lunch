...
public class NotificationsService extends FirebaseMessagingService {
    ...
    private void retrievesOtherUsersWithSameChoice() {
        ...
        UserHelper.getUsersWhoHaveSameChoice(user.getUserChoicePlaceId()).addOnSuccessListener(new OnSuccessListener<QuerySnapshot>() {
            @Override
        ...
        if (listOfUserWithSameChoice.size() != 0) {
            ...
            do {
                if (i == listOfUserWithSameChoice.size() -1) {
                    workmatesName.append(listOfUserWithSameChoice.get(i).getUserName() + ".");
                } else {
                    workmatesName.append(listOfUserWithSameChoice.get(i).getUserName() + ", ");
                }
                i++;
